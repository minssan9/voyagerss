...
const SEED_KEYS: SeedEntry[] = [
  ...
  { key: 'ANTHROPIC_API_KEY', category: 'ai', description: 'Anthropic API key (optional when using local MLX via OMP)' },
  { key: 'AIPR_AGENT_PROVIDER', category: 'ai', description: 'Agent provider: claude_cli | omp_sdk' },
  { key: 'AIPR_OMP_MODEL', category: 'ai', description: 'OMP model id (e.g. mlx-local/qwen3-coder-local)' },
  { key: 'AIPR_OMP_THINKING_LEVEL', category: 'ai', description: 'OMP thinking: off | low | medium | high | xhigh' },
  { key: 'AIPR_OMP_MAX_TURNS', category: 'ai', description: 'Max agent turns for OMP build phase' },
