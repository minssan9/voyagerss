...
import { requireAdmin, requirePermission, AuthenticatedRequest } from '@investand/middleware/authMiddleware'
...
router.get('/system/collection-status', async (req, res) => {
  try {
    ...
    const status = await DataCollectionService.getCollectionStatus(days)
...
router.get('/system/status', async (req, res) => {
  try {
    const [latestIndex, marketData, recentLogs] = await Promise.all([
      FearGreedIndexRepository.findLatest(),
      MarketDataRepository.getLatestMarketData(),
      DataCollectionService.getCollectionStatus(1)
    ])
