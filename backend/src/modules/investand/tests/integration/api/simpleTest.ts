...
try {
  ...
  const krxTypes = require('@investand/collectors/krxCollectionService')
  console.log('  ✅ KRX Collector 타입 로드 성공')
  
  // BOK 수집기 관련 타입만 import  
  const bokTypes = require('@investand/collectors/financial/bokCollector')
  console.log('  ✅ BOK Collector 타입 로드 성공')
  
  // Fear & Greed 계산기 타입만 import
  const fearGreedTypes = require('@investand/services/core/fearGreedCalculator')
  console.log('  ✅ Fear & Greed Calculator 타입 로드 성공')
  
  // 배치 처리 서비스 타입만 import
  const batchProcessingTypes = require('@investand/services/domain/BatchProcessingService')
