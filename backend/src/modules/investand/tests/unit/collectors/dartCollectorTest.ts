...
// import { DartBatchService } from '@investand/services/core/dartBatchService' // Updated path
...
describe('DART Collector Tests', () => {
  ...
  test('공시 데이터 수집 - 기본', async () => {
    ...
    try {
      ...
      expect(Array.isArray(result.stockDisclosures)).toBe(true)
   ...
   */
  /**
   * KOSPI 200 기업 코드 조회 테스트 (미구현으로 스킵)
   */
  test.skip('KOSPI 200 기업 코드 조회', async () => {
  ...
  test('API 호출 간격 제어', async () => {
    ...
    try {
      const testRequest1 = DartApiClient.fetchDisclosures({
      ...
      const testRequest2 = DartApiClient.fetchDisclosures({
  ...
  test('잘못된 날짜 형식 오류 처리', async () => {
    ...
    try {
      await DartCollectionService.collectDailyDisclosures(invalidDate)
...
describe('DART Integration Tests', () => {
  ...
  test('전체 워크플로우 시뮬레이션', async () => {
    ...
    try {
      ...
      // 1. 공시 데이터 수집
      const disclosures = await DartCollectionService.collectDailyDisclosures(testDate)
      expect(disclosures.totalDisclosures).toBeGreaterThanOrEqual(0)

      // 2. Fear & Greed 관련 필터링
      const allDisclosures = [ // stockDisclosures are DartDisclosureData[]
...
describe('DART Performance Tests', () => {
  ...
  test('대량 데이터 처리 성능', async () => {
    ...
    try {
      ...
      const kospi200Corps = ['00126380', '000660'] // Dummy
...
export async function runManualDartTests(): Promise<void> {
  ...
  try {
    ...
    const result = await DartCollectionService.collectDailyDisclosures(yesterday)
    logger.info(`수집된 공시: 총 ${result.totalDisclosures}건`)
    logger.info(`- 지분공시: ${result.stockDisclosures.length}건`)

    // 2. Fear & Greed 관련 공시 필터링
    const allDisclosures = [
      ...result.stockDisclosures
    ]
