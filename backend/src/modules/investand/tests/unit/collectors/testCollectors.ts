...
async function testCollectors() {
  ...
  try {
    ...
    const krxKospiData = await KrxApiClient.fetchStockData(testDate, 'KOSPI')
    const krxKosdaqData = await KrxApiClient.fetchStockData(testDate, 'KOSDAQ')
    const krxInvestorTradingData = await KrxApiClient.fetchInvestorTradingData(testDate)
    const krxOptionData = await KrxApiClient.fetchOptionData(testDate)
    ...
    // 4. 최근 영업일 계산 테스트
    console.log('\n📅 최근 영업일 계산 테스트...')
    const lastBusinessDay = KrxApiClient.getLastBusinessDay()
