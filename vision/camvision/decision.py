...
class DrivingDecider:
    ...
    def decide(self, report: "FrameReport") -> Decision:
        ...
        frame_area = w * h
        if frame_area <= 0:
            return Decision(action=Action.GO, reason="clear", ts=report.timestamp)
        ...
        if blind is not None and blind.triggered:
            ...
            return self._settle(raw, reason, target, report.timestamp)
        ...
        if stop_candidates:
            ...
            return self._settle(Action.STOP, reason, self._target_dict(biggest, area_ratio(biggest)),
                                 report.timestamp)
        ...
        if slow_candidates:
            ...
            steer = self._steer_away(slow_candidates, w)
            decision = self._settle(Action.SLOW, reason, self._target_dict(biggest, area_ratio(biggest)),
                                     report.timestamp)
            decision.steer = steer
            return decision

        return self._settle(Action.GO, "clear", None, report.timestamp)
