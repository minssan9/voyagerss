...
실행: python -m camvision.roi_tool [옵션]

맥/파이 어디서든 정규화 좌표(0~1)로 저장하므로, 한쪽에서 그린 roi.json 을
해상도가 다른 다른 쪽에서도 그대로 쓸 수 있다.

헤드리스(파이, GUI 없음) 환경에서 그리는 방법:
  1. 파이에서: python -m camvision.roi_tool --source picamera --capture frame.jpg
  2. frame.jpg 를 scp 로 맥에 복사
  3. 맥에서: python -m camvision.roi_tool --image frame.jpg --out roi.json
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="camvision.roi_tool", description="ROI 구역 그리기 도구")
