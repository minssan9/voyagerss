#!/usr/bin/env bash
# Render and sync Voyagerss gateway nginx from deploy/nginx/voyagerss.conf (single source).
#
...
SOURCE_FILE="${NGINX_SOURCE_FILE:-${REPO_ROOT}/deploy/nginx/voyagerss.conf}"
...
if [[ "${INSTALL_MODE}" == "reference-only" ]]; then
  ...
  echo "Merge using: ${REPO_ROOT}/deploy/nginx/GATEWAY-NGINX-MERGE-PROMPT.md"
  exit 0
fi
