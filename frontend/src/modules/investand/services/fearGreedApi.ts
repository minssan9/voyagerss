...
export const fearGreedApi = {
  ...
  async getCurrentIndex(): Promise<FearGreedIndex> {
    try {
      const response = await api.get<{ success: boolean; data: FearGreedIndex; timestamp: string }>(
  ...
  async getIndexByDate(date: string): Promise<FearGreedHistoryItem> {
    try {
      const response = await api.get<{ success: boolean; data: FearGreedHistoryItem; timestamp: string }>(
  ...
  async getStats(): Promise<FearGreedStats> {
    try {
      const response = await api.get<{ success: boolean; data: FearGreedStats; timestamp: string }>(
  ...
  async calculateIndex(date: string): Promise<FearGreedCalculationResult> {
    try {
      const response = await api.post<{ success: boolean; data: FearGreedCalculationResult }>(
  ...
  async recalculateRange(startDate: string, endDate: string): Promise<FearGreedRangeResult[]> {
    try {
      const response = await api.post<{ success: boolean; data: FearGreedRangeResult[] }>(
  ...
  async getComponents(date: string): Promise<FearGreedIndex['components']> {
    try {
      const response = await api.get<{ success: boolean; data: FearGreedIndex['components'] }>(
