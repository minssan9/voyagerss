...
```bash
# Docker 빌드 (로컬)
docker compose -f meta/deploy/docker-compose.yml --env-file .env.local build

# 로컬 실행
docker compose -f meta/deploy/docker-compose.yml --env-file .env.local up -d

# 프로덕션 (드롭릿): meta/deploy/docker-compose.prod.yml → CI가 /data/voyagerss/docker-compose.yml 로 배포
```
