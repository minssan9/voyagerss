...
```bash
bash meta/deploy/scripts/install-nginx-site.sh   # → /data/voyagerss/nginx/voyagerss.conf
# Docker gateway upstream:
VOYAGERSS_UPSTREAM_HOST=host.docker.internal bash meta/deploy/scripts/install-nginx-site.sh
```
...
- 설정 단일 원본: meta/deploy/nginx/voyagerss.conf (upstream host는 install 시 치환)
...
## 병합할 내용
meta/deploy/nginx/voyagerss.conf 전체 또는 다음을 http {} / server 블록에 맞게 분리 삽입:
